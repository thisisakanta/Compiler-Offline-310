...
varDeclare **vardeclarelist=NULL;///int a,b,c
varDeclare **globalVariable=NULL ;//global
int globalListSize=0;
...
string newLabelCounter(){
	levelCounter++;
	return "L"+to_string(levelCounter);
}
...
//FOR ICG DOING THE SECOND PREORDER TRAVERSE
void codeGeneration(int t,parseTreeNode  *root)
{
	if(root==NULL){
		return ;
	...
	if(root->getRule()=="start : program"){
		...
		for(int i=0;i<globalListSize;i++){
				...
				if(globalVariable[i]->getArraySize()!=-1){
					codeFile<<"\t"<<(globalVariable[i]->getSymbol())->getSymbolName()<<" DW "<<globalVariable[i]->getArraySize()<< " DUP (0000H)\n";
				}
				else{
					codeFile<<"\t"<<(globalVariable[i]->getSymbol())->getSymbolName()<< " DW 1 DUP (0000H)\n";
				}
		}
		codeFile<< ".CODE\n";
		parseTreeNode **child=root->getChildren();
		...
		codeFile<< newLineProc;
        codeFile<< printOutputProc;
    	codeFile<< "END main\n";
	}
	if(root->getRule()=="program : program unit"){
		
		parseTreeNode **child=root->getChildren();
		codeGeneration(t+1,child[0]);
		codeGeneration(t+1,child[1]);
	}
	if(root->getRule()=="program : unit"){
		
		parseTreeNode **child=root->getChildren();
		codeGeneration(t+1,child[0]);
		//codeGeneration(t+1,child[1]);
	}
	if(root->getRule()=="unit : var_declaration"){
		
		parseTreeNode **child=root->getChildren();
		codeGeneration(t+1,child[0]);
		//codeGeneration(t+1,child[1]);
	}
	if(root->getRule()=="unit : func_definition"){
		
		parseTreeNode **child=root->getChildren();
		codeGeneration(t+1,child[0]);
		//codeGeneration(t+1,child[1]);
	}
	if(root->getRule()=="func_definition : type_specifier ID LPAREN parameter_list RPAREN compound_statement"){
		
		parseTreeNode **child=root->getChildren();
		//codeGeneration(t+1,child[0]);
		codeFile<<child[1]->symbolin->getSymbolName()<<" PROC\n";
		  if(child[1]->symbolin->getSymbolName() == "main"){
		...
		codeGeneration(t+1,child[5]);
		string s=newLabelCounter();
		codeFile<<s<<":\n";
		if(child[1]->symbolin->getSymbolName()=="main")
		{
			codeFile<<child[1]->symbolin->getSymbolName()<<"_EXIT:\n";
		codeFile<<"\tADD SP, "<<-1*child[5]->stackSize<<"\n";
			codeFile<<"\tPOP BP\n";
			codeFile<<"\tMOV AX,4CH\n";
			codeFile<<"\tINT 21H\n";
			codeFile<<child[1]->symbolin->getSymbolName()<<" ENDP\n";
			//codeFile<<"END MAIN\n";
	...
	if(root->getRule()=="func_definition : type_specifier ID LPAREN RPAREN compound_statement"){
		...
		codeGeneration(t+1,child[4]);
		string s=newLabelCounter();
		codeFile<<s<<":\n";
		if(child[1]->symbolin->getSymbolName()=="main")
		{
			codeFile<<child[1]->symbolin->getSymbolName()<<"_EXIT:\n";
			codeFile<<"\tADD SP, "<<-1*child[4]->stackSize<<"\n";
			codeFile<<"\tPOP BP\n";
			codeFile<<"\tMOV AX,4CH\n";
			codeFile<<"\tINT 21H\n";
			codeFile<<child[1]->symbolin->getSymbolName()<<" ENDP\n";
	...
	if(root->getRule()=="var_declaration : type_specifier declaration_list SEMICOLON"){
		
		parseTreeNode **child=root->getChildren();
		//codeGeneration(t+1,child[0]);
		codeGeneration(t+1,child[1]);
		varDeclare **temp=child[1]->getSymbol();
		//cout<<"yes i am in";
		for(int i=0;i<child[1]->symbolSize;i++){
			
			if(temp[i]->globalFlag){
				//cout<<"yes i am in";
				//cout<<(temp[i]->getSymbol())->getSymbolName()<<endl;
				continue;
			}
			else{
				if(temp[i]->getArraySize()!=-1){
						codeFile<< "\tSUB SP, "<< to_string(2*temp[i]->getArraySize())<< "\n";
				}
				else{
				codeFile<<"\tSUB SP,2\n";
				}
	...
	if(root->getRule()=="compound_statement : LCURL statements RCURL"){
           parseTreeNode **child=root->getChildren();
		//codeGeneration(t+1,child[0]);
			l=0;
			codeGeneration(t+1,child[1]);
	...
	if(root->getRule()=="statements : statement"){
           parseTreeNode **child=root->getChildren();
		//codeGeneration(t+1,child[0]);
		
		
			codeGeneration(t+1,child[0]);
			root->labelEnd=child[0]->labelEnd;
		

	}
	if(root->getRule()=="statements : statements statement"){
           parseTreeNode **child=root->getChildren();
		//codeGeneration(t+1,child[0]);
		...
		codeGeneration(t+1,child[0]);
		//root->labelEnd=child[0]->labelEnd;
		child[1]->labelEnd=child[0]->labelEnd;
		
		codeGeneration(t+1,child[1]);
		root->labelEnd="";
		//cout<<root->labelEnd<<" yes";
		
		
		

	}
	if(root->getRule()=="statement : var_declaration"){
           parseTreeNode **child=root->getChildren();
		//codeGeneration(t+1,child[0]);
		l=0;
		codeGeneration(t+1,child[0]);
		...
		if(root->getRule()=="statement : expression_statement"){
           parseTreeNode **child=root->getChildren();
		//codeGeneration(t+1,child[0]);
	
		l=0;
		child[0]->labelEnd=root->labelEnd;
		codeGeneration(t+1,child[0]);
		
	}
	if(root->getRule()=="statement : compound_statement"){
		  parseTreeNode **child=root->getChildren();
		//codeGeneration(t+1,child[0]);
		string s=newLabelCounter();
		codeFile<<s<<":\n";
	...
	if(root->getRule()=="statement : FOR LPAREN expression_statement expression_statement expression RPAREN statement"){
		 parseTreeNode **child=root->getChildren();
		 child[3]->isCond=true;
		 string s=newLabelCounter();

		codeGeneration(t+1,child[2]);
		codeFile<<s<<":\n";
		codeGeneration(t+1,child[3]);
		codeFile<<child[3]->truelabel<<":\n";
		codeGeneration(t+1,child[6]);
		string p=newLabelCounter();
		codeFile<<p<<":\n";
		
		//codeFile<<"\tCMP AX, 1\n";
		codeGeneration(t+1,child[4]);
		codeFile<<"\tJMP "<<s<<"\n";
		codeFile<<child[3]->falselabel<<":\n";
		
		
	}
	
	if(root->getRule()=="statement : IF LPAREN expression RPAREN statement"){
		 parseTreeNode **child=root->getChildren();
		 child[2]->isCond=true;
		//codeGeneration(t+1,child[0]);
		string s=newLabelCounter();
		...
		codeGeneration(t+1,child[2]);
		codeFile<<child[2]->truelabel<<":\n";
		
		codeGeneration(t+1,child[4]);
		codeFile<<child[2]->falselabel<<":\n";
	}
	if(root->getRule()=="statement : IF LPAREN expression RPAREN statement ELSE statement"){
		...
		if(root->labelEnd==""){
			...
			string s=newLabelCounter();
			child[2]->labelEnd=root->labelEnd;
			child[4]->labelEnd=root->labelEnd;
			child[6]->labelEnd=root->labelEnd;
			//root->labelEnd=s;
		}
		...
		codeFile<<"\tJMP "<<root->labelEnd<<"\n";
		codeFile<<child[2]->falselabel<<":\n";
		//codeFile<<child[2]->nextlabel<<"NO:\n";
		...
		codeFile<<child[6]->labelEnd<<":\n";
		root->labelEnd=child[6]->labelEnd;
		
		
		
	}
	if(root->getRule()=="statement : WHILE LPAREN expression RPAREN statement"){
		parseTreeNode **child=root->getChildren();
		...
		codeFile<<s<<":\n";
		codeGeneration(t+1,child[2]);
		if(child[2]->truelabel==""){
			string truelabel=newLabelCounter();
			string falselabel=newLabelCounter();
			codeFile<<"\tCMP AX, 0\n";
			codeFile<<"\tJNE "<<truelabel<<"\n";
			codeFile<<"\tJMP "<<falselabel<<"\n";
			codeFile<<truelabel<<":\n";
			codeGeneration(t+1,child[4]);
			codeFile<<"\tJMP "<<s<<"\n";
			codeFile<<falselabel<<":\n";
	...
	if(root->getRule()=="statement : PRINTLN LPAREN ID RPAREN SEMICOLON"){
        string s=newLabelCounter();
		codeFile<<s<<":\n"; 
		if(root->globalFlag){
			codeFile<< "\tMOV AX, "+ (((root->getSymbol())[0]->getSymbol())->getSymbolName()) +"\n";
            codeFile<< "\tCALL PRINTNUMBER\n";
            codeFile<< "\tCALL NEWLINE\n";
			...
			if(root->stackSize>=0){
			codeFile<< "\tMOV AX, [BP+"<< root->stackSize<<"]\n";}
		else{
			codeFile<< "\tMOV AX, [BP"<< root->stackSize<<"]\n";
		}
                codeFile<< "\tCALL PRINTNUMBER\n";
	...
	if(root->getRule()=="statement : RETURN expression SEMICOLON"){
		  parseTreeNode **child=root->getChildren();
		  	codeGeneration(t+1,child[1]);
			codeFile<<"\tJMP "<<child[0]->funcName<<"_EXIT\n";

	}
	if(root->getRule()=="expression_statement : expression SEMICOLON"){
           parseTreeNode **child=root->getChildren();
		//codeGeneration(t+1,child[0]);
		...
		codeGeneration(t+1,child[0]);
		root->truelabel=child[0]->truelabel;
		root->falselabel=child[0]->falselabel;
		
		//cout<<"i am in expression statement"<<endl;

	}
	if(root->getRule()=="variable : ID"){
           parseTreeNode **child=root->getChildren();
		  //codeFile<<child[0]->symbolin->getSymbolName()<<endl;

	}
	if(root->getRule()=="variable : ID LTHIRD expression RTHIRD"){
	...
	if(root->getRule()=="expression : logic_expression"){
           parseTreeNode **child=root->getChildren();
		//codeGeneration(t+1,child[0]);
		child[0]->isCond=root->isCond;
		child[0]->labelEnd=root->labelEnd;
			//cout<<"yes  i am in logic expression"<<endl;
			//child[0]->isCond=root->isCond;
			//codeFile<<"in logic expression"<<root->isCond<<endl;
			//codeFile<<child[0]->getRule()<<endl;
			

		codeGeneration(t+1,child[0]);
		root->truelabel=child[0]->truelabel;
		root->falselabel=child[0]->falselabel;
		root->nextlabel=child[0]->nextlabel;
		
		

	}
	if(root->getRule()=="expression : variable ASSIGNOP logic_expression"){
        parseTreeNode **child=root->getChildren();
		// if(root->labelEnd==""){string s=newLabelCounter();
		// codeFile<<s<<":\n";}
		// else{
		// 	codeFile<<root->labelEnd<<
		// }
		 string s=newLabelCounter();
		codeFile<<s<<":\n"; 
		...
		codeGeneration(t+1,child[2]);
		 ...
		 //codeFile<<symbol->getSymbolName()<<"yes"<<endl;
		//codeFile<<((child[0]->getSymbol())[0]->getSymbol())->getSymbolName()<<endl;
		 
		// if(((child[0]->symbolin))!=NULL){
		// 	codeFile<<"yes form baal"<<endl;
		// 	codeFile<<child[0]->symbolin->getSymbolName()<<endl;
		// 	codeFile<<child[0]->symbolin->getSymbolType()<<endl;
		//  }
		//codeFile<<((child[0]->getSymbol())[0]->getSymbol())->getSymbolName()<<endl;
		//cout<<((child[0]->getSymbol())[0]->getSymbol())->getSymbolName()<<endl;
		
		if(child[0]->globalFlag && !((((child[0]->getSymbol())[0]->getSymbol())->getVarInfo())->getArraySize()!=-1)){
					//codeGeneration(t+1,child[0]);
					codeFile<<"\tMOV "+((child[0]->getSymbol())[0]->getSymbol())->getSymbolName()<< ", AX\n";
		}
		
		
		  else if(((((child[0]->getSymbol())[0]->getSymbol())->getVarInfo())->getArraySize()!=-1))
		 {
			if(child[0]->globalFlag){
				codeFile<<"\tPOP BX\n";
			codeFile<<"\tPUSH AX\n";
			codeFile<<"\tMOV AX,2\n";
			codeFile<<"\tMUL BX\n";
			...
			codeFile<<"\tPOP AX\n";
			//codeFile<<"\tMOV SI,BX\n";
			codeFile<<"\tMOV "<<((child[0]->getSymbol())[0]->getSymbol())->getSymbolName()<<"[BX], AX\n";
			...
			codeFile<<"\tMOV AX, "<<child[0]->stackSize<<"\n";
			codeFile<<"\tPOP BX\n";
			...
			codeFile<<"\tMOV SI,BX\n";
			codeFile<<"\tNEG SI\n";
			codeFile<<"\tMOV [BP+SI], AX\n";
			}
	}	
		else{

			if(child[0]->stackSize>=0){
			codeFile<< "\tMOV [BP+"<<child[0]->stackSize<<"], AX\n";
		}
		else{
			codeFile<< "\tMOV [BP"<<child[0]->stackSize<<"], AX\n";
		}
	...
	if(root->getRule()=="logic_expression : rel_expression"){
           parseTreeNode **child=root->getChildren();
		   child[0]->isCond=root->isCond;
		    child[0]->assign=root->assign;
		   //cout<<"in rel expression"<<root->isCond<<endl;
			codeGeneration(t+1,child[0]);
			root->truelabel=child[0]->truelabel;
		root->falselabel=child[0]->falselabel;
		root->nextlabel=child[0]->nextlabel;
	...
	if(root->getRule()=="logic_expression : rel_expression LOGICOP rel_expression"){
		...
		if(root->isCond){
			...
			parseTreeNode **child=root->getChildren();
		  // codeFile<<" rel_expression"<<root->isCond<<endl;
		   child[0]->isCond=root->isCond;
		...
		//codeFile<<"yes"<<endl;
		  ...
		  // string labelTrue=newLabelCounter();
		   if(((child[1]->getSymbol())[0]->getSymbol())->getSymbolName()=="&&"){
			codeFile<<child[0]->truelabel<<":\n";

			 codeGeneration(t+1,child[2]);
			 codeFile<<child[0]->falselabel<<":\n";
			 codeFile<<"\tJMP "<<child[0]->nextlabel<<"\n";
			 codeFile<<child[0]->nextlabel<<":\n";
			 codeFile<<"\tJMP "<<child[2]->falselabel<<"\n";
			 root->nextlabel=child[0]->nextlabel;
		...
		//  codeFile<<level<<":\n";
		   ...
		   root->truelabel=child[2]->truelabel;
		    root->falselabel=child[2]->falselabel;
		...
		string x = newLabelCounter();//1ST LEVEL check
		
        string z = newLabelCounter();//AX 1
		string y = newLabelCounter();//AX 0
        string a = newLabelCounter();//MAIN LEVEL
		parseTreeNode **child=root->getChildren();
		  // codeFile<<" rel_expression"<<root->isCond<<endl;
		child[0]->isCond=root->isCond;
		child[2]->isCond=root->isCond;
	...
	if(root->getRule()=="rel_expression : simple_expression"){
           parseTreeNode **child=root->getChildren();
		//codeGeneration(t+1,child[0]);
		child[0]->isCond=root->isCond;
		 child[0]->assign=root->assign;
	...
	if(root->getRule()=="rel_expression : simple_expression RELOP simple_expression"){
		 parseTreeNode **child=root->getChildren();
		//codeGeneration(t+1,child[0]);
		//codeFile<<"please "<<root->isCond<<endl;
		if(root->isCond){


		string labelTrue=newLabelCounter();
		string labelfalse=newLabelCounter();
		string nextlabel=newLabelCounter();
		//string labelFalse=newLabelCounter();
		child[1]->nextlabel=nextlabel;
		codeGeneration(t+1,child[2]);
		codeFile<<"\tPUSH AX\n";
		codeGeneration(t+1,child[0]);
			codeFile<<"\tPOP DX\n";

		child[0]->nextlabel=nextlabel;
		string opcode=opCode(((child[1]->getSymbol())[0]->getSymbol())->getSymbolName());
	
		codeFile<<"\tCMP AX,DX\n\t";
		
		codeFile<<opcode<<" "<<labelTrue<<"\n";
		codeFile<<"\tJMP "<<labelfalse<<"\n";
		root->truelabel=labelTrue;
		root->falselabel=labelfalse;
		root->nextlabel=nextlabel;
		//codeFile<<nextlabel<<"yes i am here"<<endl;


		// \tJMP "<<"L"<<levelCounter+1<<"\n";
		// codeFile<<labelTrue<<":\n";
	}
		else{
		codeGeneration(t+1,child[2]);
		...
		string labelTrue = newLabelCounter();
		string labelFalse = newLabelCounter();
		string a=newLabelCounter();
		codeFile<<"\tCMP AX,DX\n\t";
		codeFile<<opcode<<" "<<labelTrue<<"\n\tJMP "<<a<<endl<<labelTrue<<":\n";
		codeFile<<"\tMOV AX,1\n\tJMP "<<labelFalse<<"\n"<<a<<":\n\tMOV AX,0\n"<<labelFalse<<":\n";
		}

	}
	if(root->getRule()=="simple_expression : term"){
           parseTreeNode **child=root->getChildren();
		//codeGeneration(t+1,child[0]);
		child[0]->isCond=root->isCond;
		child[0]->assign=root->assign;
		
		//child[0]->nextlabel=root->nextlabel;
	...
	if(root->getRule()=="simple_expression : simple_expression ADDOP term"){
			parseTreeNode **child=root->getChildren();
            ...
            //codeFile<< "\tPOP AX\n";
			//cout<<((child[0]->getSymbol())[0]->getSymbol())->getSymbolName()<<endl;
            if(((child[1]->getSymbol())[0]->getSymbol())->getSymbolName()=="+") codeFile<< "\tADD AX, DX\n";
            if(((child[1]->getSymbol())[0]->getSymbol())->getSymbolName()=="-") codeFile<< "\tSUB AX, DX\n";

	}
	if(root->getRule()=="term : unary_expression"){
           parseTreeNode **child=root->getChildren();
		//codeGeneration(t+1,child[0]);
		child[0]->isCond=root->isCond;
		child[0]->assign=root->assign;
	...
	if(root->getRule()=="term : term MULOP unary_expression"){
		 parseTreeNode **child=root->getChildren();
		 codeGeneration(t+1,child[2]);
		codeFile<<"\tPUSH AX\n";
		 codeGeneration(t+1,child[0]);
		 ...
		 else if(((child[1]->getSymbol())[0]->getSymbol())->getSymbolName()=="/"){
			 codeFile<< "\tCWD\n";
            codeFile<< "\tIDIV CX\n";
          codeFile<< "\tMOV CX, AX\n";
		 }
	...
	if(root->getRule()=="unary_expression : ADDOP unary_expression"){
           parseTreeNode **child=root->getChildren();
		//codeGeneration(t+1,child[0]);
		codeGeneration(t+1,child[1]);
		   //if(((child[0]->getSymbol())[0]->getSymbol())->getSymbolName()=="+") codeFile<< "\tNEG AX\n";
            if(((child[0]->getSymbol())[0]->getSymbol())->getSymbolName()=="-") codeFile<< "\tNEG AX\n";


	}
	if(root->getRule()=="unary_expression : NOT unary_expression"){
           parseTreeNode **child=root->getChildren();
		//codeGeneration(t+1,child[0]);
		codeGeneration(t+1,child[1]);
	...
	if(root->getRule()=="unary_expression : factor"){
           parseTreeNode **child=root->getChildren();
		//codeGeneration(t+1,child[0]);
		child[0]->isCond=root->isCond;
		 child[0]->assign=root->assign;
	...
	if(root->getRule()=="factor : variable"){
           parseTreeNode **child=root->getChildren();
		codeGeneration(t+1,child[0]);
		//codeGeneration(t+1,child[0]);
		//codeGeneration(t+1,child[2]);
		if(child[0]->globalFlag && !((((child[0]->getSymbol())[0]->getSymbol())->getVarInfo())->getArraySize()!=-1)){
					//codeGeneration(t+1,child[0]);
			codeFile<<"\tMOV AX, "+((child[0]->getSymbol())[0]->getSymbol())->getSymbolName()<< "\n";
		}
		 else if(((((child[0]->getSymbol())[0]->getSymbol())->getVarInfo())->getArraySize()!=-1))
		 {
			if(child[0]->globalFlag){
				...
				codeFile<<"\tMUL BX\n";
				codeFile<<"\tMOV BX,AX\n";
				codeFile<<"\tMOV AX, "<<((child[0]->getSymbol())[0]->getSymbol())->getSymbolName()<<"[BX]\n";
			}
			else{
				codeFile<<"\tPOP BX\n";
				codeFile<<"\tMOV AX,2\n";
		...
		if(child[0]->stackSize>=0){
			codeFile<< "\tMOV AX, [BP+"<< child[0]->stackSize<<"]\n";
		}
		else{
			codeFile<< "\tMOV AX, [BP"<< child[0]->stackSize<<"]\n";
		}
	...
	if(root->getRule()=="factor : ID LPAREN argument_list RPAREN")
	{
			parseTreeNode **child=root->getChildren();
			codeGeneration(t+1,child[2]);
			codeFile<<"\tCALL "<<child[0]->symbolin->getSymbolName()<<"\n";
			codeFile<<"\tADD SP,"<<child[2]->stackSize*2<<endl;
			


	}

	if(root->getRule()=="factor : LPAREN expression RPAREN"){
		 parseTreeNode **child=root->getChildren();
		 child[1]->isCond=root->isCond;
		 //child[0]->nextlabel=root->nextlabel;
		codeGeneration(t+1,child[1]);
		root->truelabel=child[1]->truelabel;
		root->falselabel=child[1]->falselabel;
		root->nextlabel=child[1]->nextlabel;


	}
	if(root->getRule()=="factor : CONST_INT"){
           parseTreeNode **child=root->getChildren();
		//codeGeneration(t+1,child[0]);
	...
	if(root->getRule()=="factor : variable INCOP"){
		parseTreeNode **child=root->getChildren();
		codeGeneration(t+1,child[0]);
		//codeGeneration(t+1,child[0]);
		bool flag=false;
		if(child[0]->globalFlag && !((((child[0]->getSymbol())[0]->getSymbol())->getVarInfo())->getArraySize()!=-1)){
			...
			if(child[0]->globalFlag){
				...
				codeFile<<"\tMOV BX,AX\n";
				codeFile<<"\tMOV AX, "<<((child[0]->getSymbol())[0]->getSymbol())->getSymbolName()<<"[BX]\n";
				if(root->isCond||root->assign){
				codeFile<<"\tMOV CX,AX\n";
			}
			codeFile<< "\tINC AX\n";
				if(((((child[0]->getSymbol())[0]->getSymbol())->getVarInfo())->getArraySize()!=-1)){
				//codeFile<<"yes"<<endl;
				codeFile<<"\tMOV "<<((child[0]->getSymbol())[0]->getSymbol())->getSymbolName()<<"[BX], AX\n";
	...
	if(root->getRule()=="factor : variable DECOP"){
		parseTreeNode **child=root->getChildren();
		//codeGeneration(t+1,child[0]);
		codeGeneration(t+1,child[0]);
		if(child[0]->globalFlag && !((((child[0]->getSymbol())[0]->getSymbol())->getVarInfo())->getArraySize()!=-1)){
		...
		codeFile<< "\tDEC AX\n";
		if(child[0]->globalFlag) codeFile<< "\tMOV "<< ((child[0]->getSymbol())[0]->getSymbol())->getSymbolName()<<", AX\n";
		 else{
	...
	if(root->getRule()=="argument_list : arguments"){
		parseTreeNode **child=root->getChildren();
		//codeGeneration(t+1,child[0]);
		codeGeneration(t+1,child[0]);
	}
	if(root->getRule()=="arguments : arguments COMMA logic_expression"){
		parseTreeNode **child=root->getChildren();
		 codeGeneration(t+1,child[0]);
     	codeGeneration(t+1,child[2]);
        codeFile<< "\tPUSH AX ;    func param\n";
	}
	if(root->getRule()=="arguments : logic_expression"){
		parseTreeNode **child=root->getChildren();
		 codeGeneration(t+1,child[0]);
        codeFile<< "\tPUSH AX ;    func param\n";
	}
...
%token<symbolInfo> ID CONST_INT CONST_FLOAT INCOP DECOP
...
func_definition : type_specifier ID  LPAREN parameter_list RPAREN {currFunc=$2->getSymbolName();}compound_statement
				{
					parseTreeNode* id=new parseTreeNode();
					id->setFirstLine(@2.first_line);
					id->setLasttLine(@2.last_line);
		...
		| type_specifier ID LPAREN RPAREN {currFunc=$2->getSymbolName();}compound_statement
		{
					parseTreeNode* id=new parseTreeNode();
					id->setFirstLine(@2.first_line);
					id->setLasttLine(@2.last_line);
			...
			LCURL
				{
						...
						if(funParmasSize>0)
						{
							...
							for(int i=0;i<funParmasSize;i++)
							{
								...
								if(temp==NULL)
								{
									table->setStackSize(table->getStackSize()-2);
									symbol->stackSize=(funParmasSize-i)*2+2;
									//cout<<"inserted"<<endl;
									cout<<symbol->getSymbolName()<<"yes var"<<endl;
									table->insert(symbol);//variable insert in the symbolTable;
								}
							}
							cout<<table->getStackSize()<<"stack"<<endl;
							table->setStackSize(0);
						}
					...
					for(int i=0;i<vardeclarelistSize;i++)
					{
						...
						else if(symbol==NULL)
						{
							...
							if(table->getCurrentScopeId()=="1"){
								temp2->globalFlag=true;
								temp2->stackSize=0;
								vardeclarelist[i]->stackSize=0;
								varDeclare **temp=new varDeclare*[globalListSize+2];
				
				//vardeclarelist=new VarDeclare[vardeclarelistSize+2];
							if(globalListSize){
								for(int i=0;i<globalListSize;i++)
									{
										temp[i]=globalVariable[i];
									}
								}
							else{
									temp[0]=NULL;
					//cout<<"yes from id";
							...
							varDeclare *temp2=vardeclarelist[i];
				//cout<<$1->getSymbolName()<<$1->getSymbolType()<<endl;
				
							temp[globalListSize]=temp2;
							globalListSize++;
							globalVariable=temp;
							vardeclarelist[i]->globalFlag=true;


							}
							else{
								//cout<<"yes in new scope";
								//cout<<vardeclarelistSize<<"size"<<endl;
								
								vardeclarelist[i]->globalFlag=false;
								if(vardeclarelist[i]->getArraySize()!=-1){
									table->setStackSize(table->getStackSize()-2*vardeclarelist[i]->getArraySize());
								}else{
								table->setStackSize(table->getStackSize()-2);
								}
								//cout<<table->getStackSize()<<" yres"<<endl;
								vardeclarelist[i]->stackSize=table->getStackSize();
								temp2->stackSize=table->getStackSize();
								//cout<<($2->getSymbol())[0]->globalFlag<<"please"<<endl;
							}
							table->insert(temp2);
							//cout<<table->getStackSize()<<"this is stacksize\n";
							//vardeclarelistSize=0;
							//cout<<temp2->stackSize<<endl;
						}
...
declaration_list : 
			declaration_list COMMA ID//int a,b;
			{
				parseTreeNode* comma=new parseTreeNode();
				comma->setFirstLine(@2.first_line);
				comma->setLasttLine(@2.last_line);
			...
			if(flag)
			{
				...
				$$->setSymbol(vardeclarelist);
				$$->symbolSize=vardeclarelistSize;
			}
...
statement : 
	  ...
	  | PRINTLN LPAREN ID RPAREN SEMICOLON
	  {
			...
			$$->addChildren(rparen);
			$$->addChildren(semicolon);
			varDeclare **temp2=new varDeclare*[2];
			temp2[0]=new varDeclare();
			...
			if(temp!=NULL){
				temp2[0]->setSymbol(temp);
				$$->stackSize=temp->stackSize;
				if(temp->globalFlag){$$->globalFlag=true;}
...
expression_statement 	: 
			 expression SEMICOLON
			{
				...
				$$->setFirstLine(@$.first_line);
				$$->setLasttLine(@$.last_line);
				//$$->setDataType($1->getDataType());//for ICG expression datatype setting
...
variable : 
			...
			if(symbol==NULL)
			 {
				...
				{
					cout<<symbol->getSymbolName()<<"yes i am done"<<endl;
					id->setDataType((symbol->getVarInfo())->getDataType());
			 		$$->setDataType((symbol->getVarInfo())->getDataType());
					varDeclare **temp=new varDeclare*[2];
					temp[0]=new varDeclare();
					//temp[1]=new varDeclare();
					temp[0]->setSymbol(symbol);
					$$->setSymbol(temp);
					$$->symbolSize=1;
					$$->stackSize=symbol->stackSize;
					//codeFile<<"yes"<<endl;
					//cout<<"stacksize ::"<<symbol->stackSize<<" "<<symbol->getSymbolName()<<endl;
					$$->globalFlag=symbol->globalFlag;
					$$->symbolin=symbol;
					$$->temporary.push_back(symbol);
					//id->symbolin=symbol;
					//delete[] temp;
				}
	 ...
	 | ID LTHIRD expression RTHIRD
	 {
			...
			$$->setFirstLine(@$.first_line);
			$$->setLasttLine(@$.last_line);
			$$->setRule("variable : ID LTHIRD expression RTHIRD");
			 ...
			 if(symbol->getSymbolType()=="VAR" && (symbol->getVarInfo())->getArraySize()>-1)
			{
				...
				//id->symbolin=symbol;
				//cout<<$$->symbolin->getSymbolName()<<"AHON\n";
 ...
 expression : 
	   ...
	   | variable ASSIGNOP logic_expression
	   {
			...
			$$->addChildren(assign);
			$$->addChildren($3);
			varDeclare **temp=$1->getSymbol();
			SymbolInfo *symbol=temp[0]->getSymbol();
			//cout<<symbol->getSymbolName()<<endl;
			//parseTreeNode **child=$$->getChildren();
			// cout<<(child[0]->symbolin)->getSymbolName()<<"ahon2\n";
			// cout<<(child[0]->symbolin)->getSymbolType()<<"ahon2\n";
			

			
			//cout<<(($1->getSymbol())[0]->getSymbol())->getSymbolName()<<"yes two\n";
...
simple_expression : 
		...
		| simple_expression ADDOP term
		{
			...
			temp[0]=new varDeclare();
			temp[0]->setSymbol($2);
			addNode->setSymbol(temp);
			addNode->symbolSize=1;
			addNode->stackSize=$2->stackSize;
			addNode->globalFlag=$2->globalFlag;
...
term :	
     ...
     |  term MULOP unary_expression
	 {
			...
			mulNode->setRule("MULOP : "+$2->getSymbolName());
			mulNode->setsymbolinfo();
			varDeclare **temp=new varDeclare*[2];
				temp[0]=new varDeclare();
				temp[0]->setSymbol($2);
				mulNode->setSymbol(temp);
				mulNode->symbolSize=1;
				mulNode->stackSize=$2->stackSize;
				mulNode->globalFlag=$2->globalFlag;
...
unary_expression : 
		ADDOP unary_expression
		{
			...
			varDeclare **temp=new varDeclare*[2];
				...
				addNode->setSymbol(temp);
				addNode->symbolSize=1;
				addNode->stackSize=$1->stackSize;
				addNode->globalFlag=$1->globalFlag;
			$$=new parseTreeNode();
			$$->setFirstLine(@$.first_line);
			$$->setLasttLine(@$.last_line);
